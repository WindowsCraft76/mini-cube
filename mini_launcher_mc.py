...
class MiniLauncherApp:
    def __init__(self, root):
        ...
        self.log_buffer = []  # conserve les logs quand la fenêtre est fermée
        ...
        # Bouton pour afficher/cacher la fenêtre de logs
        self.show_logs_btn = tk.Button(root, text="Afficher les logs", command=self.toggle_logs_window)
        ...
        # NOTE: plus de zone de logs dans la fenêtre principale — logs séparés dans la Toplevel
    ...
    def _append_log_to_window(self, msg, kind="info"):
        if self.log_text_win:
            try:
                ...
                # si erreur (fenêtre peut avoir été fermée entre-temps), on ignore
                pass

    # Crée la fenêtre de logs (ou la détruit si déjà ouverte)
    def toggle_logs_window(self):
        if self.log_window and tk.Toplevel.winfo_exists(self.log_window):
            ...
            self.log_window = None
            self.log_text_win = None
            self.show_logs_btn.config(text="Afficher les logs")
        else:
            # créer la fenêtre
            self.log_window = tk.Toplevel(self.root)
            self.log_window.title("Logs du launcher")
            self.log_window.geometry("800x400")
            # Quand l'utilisateur ferme via la croix, appeler la même logique
            ...
            self.show_logs_btn.config(text="Cacher les logs")

    def _on_close_log_window(self):
        # Détruire proprement la fenêtre et mettre à jour le bouton
        if self.log_window:
            try:
                self.log_window.destroy()
            except Exception:
    ...
    def update_progress(self, done, total, text=""):
        ...
        # utiliser after pour la sécurité thread
    ...
    def prepare_version(self, version_id):
        ...
        for kind, path, url in tasks:
            if self.cancel_download:
                self.log("Téléchargement annulé !", "info")
            ...
            if path.exists():
                ...
                try:
                    ...
                    self.log(f"Erreur téléchargement {url}: {e}", "error")
            done += 1
    ...
    def launch_game(self):
        if self.download_thread and self.download_thread.is_alive():
            ...
            self.log("Annulation du téléchargement demandée.", "warn")
            return
    ...
    def _launch_game_thread(self):
        ...
        try:
            ...
            self.log(f"[ERROR] Impossible de lancer le processus Java: {e}", "error")
            self.root.after(0, lambda: self.launch_btn.config(text="Lancer le jeu"))
            self.root.after(0, lambda: self.set_ui_state(True))
            return

        for line in iter(game_process.stdout.readline, ''):
            # On envoie les lignes dans le log (thread-safe)
